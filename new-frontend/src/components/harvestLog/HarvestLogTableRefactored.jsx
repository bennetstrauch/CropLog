  ...
  return (
    <div className="bg-white w-full h-full min-h-screen">
      ...
      <div className="px-4 py-4 border-b border-gray-200">
        ...
        {/* DESKTOP layout (≥ sm): original 3-column single row */}
        <div className="hidden sm:flex items-start justify-between gap-4">
          {/* Left: back + title + count + date range */}
          <div className="flex items-start gap-3">
            <button
              onClick={() => navigate('/')}
              className="text-gray-500 hover:text-gray-700 transition-colors duration-200 flex-shrink-0"
              style={{ background: 'none', border: 'none', padding: 0, marginTop: 4, borderRadius: 0 }}
        ...
        {/* MOBILE layout (< sm): 2-row stacked */}
        <div className="flex flex-col gap-2 sm:hidden">
          {/* Row 1: back + title | Summary + CSV */}
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <button
                onClick={() => navigate('/')}
                className="text-gray-500 hover:text-gray-700 transition-colors duration-200 flex-shrink-0"
                style={{ background: 'none', border: 'none', padding: 0, borderRadius: 0 }}
                title="Back to main page"
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            ...
            {!loading && sortedEntries.length > 0 && (
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  onClick={handleExportCsv}
                  className="px-3 py-1.5 !bg-teal-700 text-white text-sm font-semibold rounded-lg hover:!bg-blue-800 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 flex items-center gap-1.5"
                  title={`Export current ${isSummaryMode ? 'summary' : 'detail'} view as CSV`}
                >
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
          ...
          {/* Row 2: date range | timeframe nav */}
          <div className="flex items-center justify-between gap-2">
            <div className="relative">
              <div
                className="cursor-pointer hover:opacity-70 transition-opacity text-sm"
                onClick={() => setShowPicker(p => !p)}
                title="Click to set custom date range"
              >
                <DateRangeDiv {...dateRange} />
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {summaryEntries.map((row, index) => (
                <tr key={row.cropId} className={`transition-colors duration-150 ${index % 2 === 0 ? 'bg-white hover:bg-gray-50' : 'bg-yellow-50 hover:bg-yellow-100'}`}>
                  <td className="px-2 py-2 sm:px-6 sm:py-4 text-sm font-medium text-gray-900">{row.cropName}</td>
                  <td className="px-2 py-2 sm:px-6 sm:py-4 text-sm text-gray-900">{row.totalQuantity}</td>
                  <td className="px-2 py-2 sm:px-6 sm:py-4 text-sm text-gray-500">{row.measureUnitName}</td>
                  <td className="px-2 py-2 sm:px-6 sm:py-4 text-sm text-gray-700">
                  ...
                  <td className="px-2 py-2 sm:px-6 sm:py-4 text-sm text-gray-500">{row.categoryName}</td>
                  <td className="px-2 py-2 sm:px-6 sm:py-4 text-sm text-gray-500">{row.entryCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
